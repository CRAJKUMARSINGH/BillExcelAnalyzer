...
console.log('🚀 PROCESSING ALL 15 INPUT TEMPLATES');
console.log('='.repeat(40));

// Directories
const inputDir = path.join(__dirname, '..', 'TEST_INPUT_FILES');
const outputDir = path.join(__dirname, '..', 'demonstration_output', 'all_15_files');
...
// Generate comprehensive HTML report with first page and deviation includes
const generatePDFReport = () => {
  ...
  <head>
    <meta charset="UTF-8">
    <title>Bill Excel Analyzer - 15 Files Analysis Report</title>
    <style>
      body {
  ...
  <body>
    ...
    <div class="header">
      ...
      <p>Comprehensive Analysis of 15 Input Templates</p>
    ...
    <div class="first-page-content">
      ...
      <p>This report provides a comprehensive bird's eye assessment of all 15 input templates processed by the Bill Excel Analyzer system. Each file has been examined for its structure, content, and compliance with statutory requirements.</p>
      
      <div class="deviation-note">
        <h3 style="color: #856404; margin-top: 0;">🔍 Deviation Analysis Summary</h3>
        <p>Deviation statements and extra items have been analyzed to ensure proper handling of variations from the original contract scope. ${reportData.filter(item => item.totalExtraItems > 0).length} out of ${inputFiles.length} files contain extra items/deviations.</p>
      </div>
    </div>
  ...
  // Add each file's detailed information with emphasis on first page and deviations
  reportData.forEach((fileData, index) => {
    ...
    <div class="file-section">
      ...
      <div class="section">
        <h3 class="section-title">📚 Document Structure</h3>
        <table>
          <tr>
            <th>Sheet Type</th>
            <th>Sheet Name</th>
          ...
          <tr>
            <td>Total Sheets</td>
            <td>${fileData.sheetNames.length} (${fileData.sheetNames.join(', ')})</td>
          </tr>
        </table>
      </div>
      ...
      <div class="section">
        <h3 class="section-title">📌 Deviation Analysis & Extra Items</h3>
        ${
          fileData.totalExtraItems > 0 ? 
          `<div class="deviation-table">
            <p><strong>Deviation Summary:</strong> This file contains ${fileData.totalExtraItems} extra items/deviations from the original contract scope.</p>
            <table>
              <thead>
                <tr>
          ...
          `<p>No extra items or deviations found in this file. All work appears to be within the original contract scope.</p>`
        }
      </div>
    </div>
  ...
  html += `
    <div class="footer">
      <p>Bill Excel Analyzer Comprehensive Report - 15 Files Analysis</p>
...
if (reportData.length > 0) {
  const htmlReport = generatePDFReport();
  const reportPath = path.join(reportDir, '15_files_analysis_report.html');
  ...
  console.log(`   • Files with deviations: ${reportData.filter(item => item.totalExtraItems > 0).length}`);
  
  // Also create a simplified text report
  const textReportPath = path.join(reportDir, '15_files_analysis_summary.txt');
  let textReport = `BILL EXCEL ANALYZER - 15 FILES ANALYSIS SUMMARY\n`;
  ...
  textReport += `• Files with deviations: ${reportData.filter(item => item.totalExtraItems > 0).length}\n\n`;
